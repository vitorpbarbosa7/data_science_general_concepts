# code from https://lizliz.github.io/teaspoon/_modules/teaspoon/SP/adaptivePart.html
"""
...
class Partitions:
    ...
    #    numBins = r ** 2
        ...
        log.debug(f'Number of bins: {idx}')

        # extract the points in the bin
        Xsub = data[idx, 0]
        Ysub = data[idx, 1]

        log.debug(f'Points in the bin {Xsub} and {Ysub}')
    ...
    #    print(Xsub.shape, '\t', Ysub.shape)
        ...
        log.debug(f'indices: {idx_x} & {idx_y}')
        ...
        log.debug(f'Subpartitions {ai} & {bj}')
